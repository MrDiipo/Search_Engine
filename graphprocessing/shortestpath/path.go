...
// NewCalculator returns a new shortest power calculator from
// a single vertex in a connected graph.
...
// SetExecutorFactory configures the calculator to use the custom executor
...
// CalculateShortestPaths finds the shortest path costs from srcID to all other vertices
// in the graph
...
func (c *Calculator) ShortestPathTo(dstID string) ([]string, int, error) {
	...
	if !exists {
		return nil, 0, xerrors.Errorf("unknown vertex withID %q", dstID)
	}
	var (
		minDst = v.Value().(*PathState).minDst
		path   []string
	)
...
func (c Calculator) findShortestPath(g *bspgraph.Graph, v *bspgraph.Vertex, msgIt message.Iterator) error {
	...
	// Process cost messages from neighbors and update
	// if we receive a better path announcement
	var via string
	for msgIt.Next() {
	...
	// We are done unless we receive better announcement
	v.Freeze()
	return nil
}
