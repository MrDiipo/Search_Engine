...
// makeComputeFunc returns a ComputeFn that executes the PageRank calculation
...
func makeComputeFunc(dampingFactor float64) bspgraph.ComputeFunc {
	return func(g *bspgraph.Graph, v *bspgraph.Vertex, msgIt message.Iterator) error {
		...
		switch superstep {
		case 1:
			// At step 1 we evenly distribute the PageRank scores across all vertices.
			// As the sum of all scores should be equal t 1, each vertex is assigned an
			// initial score of 1/pagecount.
		...
		// If this is a dead-end (link with no outgoing links) we treat this link
		// as if it was being connected to all links in the graph.
		// Since we cannot broadcast a message to all vertices, we will add the
		// per-vertex residual score to an accumulator and integrate it into the scores
		// calculated over the next round.
		numOfLinks := float64(len(v.Edges()))
		if numOfLinks == 0.0 {
			g.Aggregator(residualOutputAccName(superstep)).Aggregate(newScore / pageCount)
			return nil
		}
		// Otherwise, evenly distribute this node's score to all it's neighbors
		return g.BroadcastToNeighbors(v, IncomingScoreMessage{newScore / numOfLinks})
	}
}
